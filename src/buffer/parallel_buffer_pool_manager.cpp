...
auto ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) -> Page * {
  ...
  for (size_t i = 0; i < v_.size(); i++) {
    auto page = GetBufferPoolManager(starting_index_)->NewPage(page_id);
    starting_index_ = (starting_index_ + 1) % v_.size();
    if (page != nullptr) {
      return page;
    }
  }
