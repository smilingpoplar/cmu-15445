...
class RowMatrix : public Matrix<T> {
 public:
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    memcpy(this->linear_, source.data(), sizeof(T) * source.size());
  }
...
class RowMatrixOperations {
 public:
  ...
  static auto Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) -> std::unique_ptr<RowMatrix<T>> {
    ...
    auto ret = std::make_unique<RowMatrix<int>>(rows, cols);
  ...
  static auto Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) -> std::unique_ptr<RowMatrix<T>> {
    ...
    if (a_cols != matrixB->GetRowCount()) {
      return nullptr;
    }

    const int a_rows = matrixA->GetRowCount();
    const int b_cols = matrixB->GetColumnCount();
    auto ret = std::make_unique<RowMatrix<int>>(a_rows, b_cols);
    for (int r = 0; r < a_rows; r++) {
      for (int c = 0; c < b_cols; c++) {
        int sum = 0;
        for (int k = 0; k < a_cols; k++) {
          sum += matrixA->GetElement(r, k) * matrixB->GetElement(k, c);
        }
  ...
  static auto GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB, const RowMatrix<T> *matrixC)
    ...
    auto mult = Multiply(matrixA, matrixB);
    return Add(mult.get(), matrixC);
  }
};
}  // namespace bustub
